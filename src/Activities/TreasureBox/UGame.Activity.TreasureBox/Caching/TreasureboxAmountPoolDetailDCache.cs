...
public record AmountPoolBo(DateTime StarTime, DateTime EndTime);

/// <summary>
/// 红包明细
/// </summary>
public class TreasureboxAmountPoolDetailDCache : RedisHashExpireClient
{
    ...
    public TreasureboxAmountPoolDetailDCache(string operatorId, string boxId, DateTime date)
    {
        if (string.IsNullOrWhiteSpace(operatorId))
            throw new CustomException("TreasureboxAmountPoolDetailDCache: operatorId不能为空");
    ...
    /// <returns></returns>
    public async Task<(long amount, AmountPoolBo bo)> AmountDecrementAsync(long amount)
    {
        await CreateAsync();
        ...
        var count = (long)await Database.ScriptEvaluateAsync(luaScript, new RedisKey[] { RedisKey }, new RedisValue[] { amount });
    ...
    /// </summary>
    /// <returns></returns>
    public async Task<long> GetAmountPool()
    {
        var hash = await GetFiledKey();
        if (string.IsNullOrWhiteSpace(hash.filed)) return 0L;
        var amount = await Database.HashGetAsync(RedisKey, hash.filed);
        if (!amount.HasValue) return 0L;
        return (long)amount;
    }
    ...
    /// </summary>
    /// <returns></returns>
    private async Task<(string filed, AmountPoolBo bo)> GetFiledKey()
    {
        ...
        var timePeriods = (from key in keys
                ...
                into times
                           ...
                           select new AmountPoolBo(start, end))
            .ToList();
        ...
        var bo = time == null ? null : new AmountPoolBo(time.StarTime, time.EndTime);
        return (filedKey, bo);
    }
    ...
    public async Task CreateAsync()
    {
        ...
        {
            var repostiroy = DbUtil.GetRepository<Sa_treasurebox_amount_pool_detailPO>();
            ...
            if (eos.Count == 0)
            {
                ...
                var configs = TreasureBoxMemoryCacheUtil.GetTreasureBoxAmountConfig(OperatorId, BoxId);
                if (configs == null || configs.Count == 0) return;

                var details = configs.Select(w => new Sa_treasurebox_amount_pool_detailPO
                {
                    BoxID = w.BoxID,
                    OperatorID = OperatorId,
    ...
    /// <returns></returns>
    private async Task SetBaseValues(List<Sa_treasurebox_amount_pool_detailPO> eos)
    {
        var dict = new Dictionary<string, object>();
        var date = CurrDateTime.Date;
        eos.ForEach(eo => { dict[$"{eo.StartTime:yyyyMMddHH:mm:ss}-{eo.EndTime:yyyyMMddHH:mm:ss}"] = eo.Amount; });
